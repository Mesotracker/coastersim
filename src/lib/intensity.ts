...
export function analyzeRideIntensity(
  ...
  let minG = 1.0;
  let maxSpeed = 0;
  let peakLat = Math.max(0.1, maxLatEstimated);
  const hazards: HazardWarning[] = [];

  // Filter 3-sample moving window to measure sustained physiological forces (ASTM F2291 standard)
  const smoothedG: number[] = [];
  const n = telemetry.length;
  for (let i = 0; i < n; i++) {
    const pt = telemetry[i];
    if (pt.speed > maxSpeed) maxSpeed = pt.speed;
    if (pt.lat !== undefined && Math.abs(pt.lat) > peakLat) {
      peakLat = Math.abs(pt.lat);
    }

    // 3-point moving median / average to filter transient 1-frame sensor singularities
    const gPrev = i > 0 ? telemetry[i - 1].g : pt.g;
    const gNext = i < n - 1 ? telemetry[i + 1].g : pt.g;
    const sorted = [gPrev, pt.g, gNext].sort((a, b) => a - b);
    const sustainedG = sorted[1]; // median
    smoothedG.push(sustainedG);

    if (sustainedG > maxPosG) maxPosG = sustainedG;
    if (sustainedG < minG) minG = sustainedG;
  }

  // Peak negative airtime G and peak lateral G
  const negG = minG;
  const latG = peakLat;

  // Check for DEADLY thresholds (lethal physiological trauma)
  let isDeadly = false;
  let isUnsafe = false;
  ...
  if (maxPosG >= 9.0) {
    ...
    hazards.push({
      type: 'crush',
      severity: 'fatal',
      title: 'Lethal Spinal & Cardiac Collapse (+9.0G+)',
    ...
    hazards.push({
      type: 'blackout',
      severity: 'critical',
      title: 'Severe G-LOC & Retinal Blackout (+6.2G+)',
  ...
  if (negG <= -3.2) {
    ...
    hazards.push({
      type: 'redout',
      severity: 'fatal',
      title: 'Fatal Cerebral Hemorrhage (-3.2G)',
    ...
    hazards.push({
      type: 'redout',
      severity: 'critical',
      title: 'Dangerous Redout & Vascular Trauma (-2.0G)',
  ...
  if (latG >= 4.0) {
    ...
    hazards.push({
      type: 'whiplash',
      severity: 'fatal',
      title: 'Internal Decapitation / Cervical Spine Severance (4.0G+ Lat)',
    ...
    hazards.push({
      type: 'whiplash',
      severity: 'critical',
      title: 'Severe Cervical Whiplash (2.6G+ Lat)',
  ...
  if (maxSpeed >= 135 && (maxPosG > 6.0 || latG > 2.2)) {
    isUnsafe = true;
    hazards.push({
      type: 'jerk',
      severity: 'warning',
      title: 'Supersonic Kinetic Stress (135+ mph)',
